...
public class Flecha extends JPanel implements ChangeListener {
    ...
    private Punto calcularInterseccion(float yEnti, float yIsA, float xEnti, float xIsA, boolean esPadre, int anchoRect) {
        ...
        Punto cIsa = new Punto(xIsA, yIsA);//IsA
        Punto cEnt = new Punto(xEnti, yEnti);//Entidad
        Recta rCentros = null;
        try {
        ...
        if (esPadre) {
            ...
            Punto inf = new Punto(xIsA, (double) yIsA + tri3);
    ...
    private boolean checkInteresct(int ri, Punto interseccion, Punto izq, Punto dech, Punto inf) {
        ...
        try {
            ...
            if (r.pasaPor(interseccion)) {
                if (ri == 1) {
                    return (!(interseccion.x < izq.x)) && (!(interseccion.x > dech.x));
                } else if (ri == 2) {
                    return (!(interseccion.x < inf.x)) && (!(interseccion.x > dech.x))
                            && (!(interseccion.y < dech.y)) && (!(interseccion.y > inf.y));
                } else if (ri == 3) {
                    return (!(interseccion.x > inf.x)) && (!(interseccion.x < izq.x))
