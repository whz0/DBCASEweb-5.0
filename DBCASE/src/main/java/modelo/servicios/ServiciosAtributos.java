...
public class ServiciosAtributos {
    ...
    public void anadirAtributo(Vector v) {
        ...
        for (int i = 0; i < tap.getListaComponentes().size(); i++)
            if (daoAtributos.nombreDeAtributo((Integer.parseInt((String) tap.getListaComponentes().get(i)))).equalsIgnoreCase(tah.getNombre())) {
            ...
            while (k < listaAtributos.size()) {
                ...
                if (idAtributo == id_posible) {
                    ...
                    for (int i = 0; i < te.getListaAtributos().size(); i++)
                        if (daoAtributos.nombreDeAtributo((Integer.parseInt((String) te.getListaAtributos().get(i)))).equalsIgnoreCase(nuevoNombre)
                                && i != k) {
            ...
            while (k < listaAtributos.size()) {
                ...
                if (idAtributo == id_posible) {
                    ...
                    for (int i = 0; i < tr.getListaAtributos().size(); i++)
                        if (daoAtributos.nombreDeAtributo((Integer.parseInt((String) tr.getListaAtributos().get(i)))).equalsIgnoreCase(nuevoNombre)
                                && i != k) {
                            controlador.mensajeDesde_SA(TC.SA_RenombrarAtributo_ERROR_NombreDeAtributoYaExiste, v);
                            return;
                        }
            ...
            while (k < listaSubatributos.size() && !encontrado) {
                ...
                if (idAtributo == id_posible) {
                    ...
                    for (int i = 0; i < listaSubatributos.size(); i++)
                        if (daoAtributos.nombreDeAtributo((Integer.parseInt((String) listaSubatributos.get(i)))).equalsIgnoreCase(nuevoNombre)
                                && i != k) {
                            controlador.mensajeDesde_SA(TC.SA_RenombrarAtributo_ERROR_NombreDeAtributoYaExiste, v);
                            return;
                        }
    ...
    public void anadirRestriccion(Vector v) {
        ...
        if (daoAtributoes.modificarAtributo(ta))
    ...
    public void quitarRestriccion(Vector v) {
        ...
        if (daoAtributoes.modificarAtributo(te))
