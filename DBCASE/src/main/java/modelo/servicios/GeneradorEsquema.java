...
public class GeneradorEsquema {
    ...
    private final Hashtable<Integer, Tabla> tablasEntidades = new Hashtable<Integer, Tabla>();
    private final Hashtable<Integer, Tabla> tablasRelaciones = new Hashtable<Integer, Tabla>();
    private final Vector<Tabla> tablasMultivalorados = new Vector<Tabla>();
    private final Hashtable<Integer, Enumerado> tiposEnumerados = new Hashtable<Integer, Enumerado>();
                ...
                for (int m = 0; m < veya.size(); m++) {
                    ...
                    if (eya.getFinalRango() > 1) tabla.aniadeListaClavesPrimarias(primarias);
                        ...
                        if (soloHayUnos && esLaPrimeraDel1a1) {
                            ...
                            for (String[] clave : ent.getPrimaries())
    ...
    public void exportarCodigo(String text, boolean sql) {
        ...
        if (sql)
            jfc.setFileFilter(new FileNameExtensionFilter(this.msgSrc.getMessage("textosId.sqlFiles", null, this.loc), "sql"));
    ...
    private Vector<String[]> atributoCompuesto(TransferAtributo ta, String nombreEntidad, String procedencia) {
        ...
        for (int i = 0; i < subs.size(); i++) {
            ...
            if (aux.getCompuesto()) {
                ...
                lista.addAll(this.atributoCompuesto(aux, nombreEntidad, p));
            } else {
                //caso base
