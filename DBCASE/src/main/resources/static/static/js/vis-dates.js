                        ...
                        entitiesOfRelation.forEach(function (eRelation) {
                            ...
                            if (!nodes.get(eRelation.id).IsSuperEntity) {
                                if (nodes.get(eRelation.id).superEntity < 0) {
                                    ...
                                    actionHistory.push({
                                        type: 'addToNewSuperEntity',
                                        node: JSON.parse(JSON.stringify(nodes.get(eRelation.id)))
                                    });
                                    ...
                                    relationAttr.forEach(function (attr) {
                                        ...
                                        actionHistory.push({
                                            type: 'addToNewSuperEntity',
                                            node: JSON.parse(JSON.stringify(nodes.get(attr.id)))
                                        });
                                        ...
                                        subAttr.forEach(function (sAttr) {
                                            ...
                                            actionHistory.push({
                                                type: 'addToNewSuperEntity',
                                                node: JSON.parse(JSON.stringify(nodes.get(sAttr.id)))
                                            });
...
function addRelation(nombre, action, idSelected, origin = "front") {
    ...
    updateIdCount();
    var data_element = {
        id: idCount, size: tam, label: nombre, shape: 'diamond', IsSuperEntity: false, superEntity: -1,
        color: {
            border: '#c9280e',
            background: '#FF3F20',
            highlight: {
        ...
        //color: '#FF3F20'
        , scale: 20, physics: false, zIndex: 0
    };//D5FF04  cambiado(ff554b)
...
function addIsA() {    //TODO: Añadir opcion de editar en todas las opciones del elemento
    updateIdCount();
    var data_element = {
        id: idCount, label: 'IsA', shape: 'triangleDown', IsSuperEntity: false, superEntity: -1,
        color: {
            border: '#FF952A',
            background: '#FF952A',
            highlight: {
...
function addAttribute(name, action, idSelected, idEntity, pk, comp, notNll, uniq, multi, dom, sz) {
    ...
    var data_element = {
        id: idCount,
        width: 3,
        widthConstraint: {minimum: 50, maximum: 160},
        labelBackend: name,
        label: word_pk,
        dataAttribute: {
            entityWeak: valueEntityWeak,
            primaryKey: pk,
            composite: comp,
            notNull: notNll,
            unique: uniq,
    ...
    if (action == "edit") {
        ...
        if (inSuperEntity(parseInt(idEntity))) {
            ...
            actionHistory.push({
                type: 'addToNewSuperEntity',
                node: JSON.parse(JSON.stringify(nodes.get(data_element.id)))
            });
...
function addEntitytoRelation(idTo, element_role, cardinality, roleName, minCardinality, maxCardinality, action, idSelected, partActive) {
    ...
    var data_element = {
        width: 3,
        from: parseInt(idSelected),
        to: parseInt(idTo),
        label: labelText,
        labelFrom: right,
        labelTo: left,
        name: center,
        participation: partActive,
        participationFrom: minCardinality,
        participationTo: maxCardinality,
        state: "false",
        smooth: false,
        arrows: {to: {enabled: direct1}}
    };
    var data_element1 = {
        width: 3,
        from: parseInt(idSelected),
    ...
    if (action == "edit") {
        ...
        if (idEdge != null) {
            ...
            actionHistory.push({
                type: 'addNewEntityToRelation',
                edge: JSON.parse(JSON.stringify(edges.get(data_element1.id)))
            });
            ...
            actionHistory.push({
                type: 'addNewEntityToRelation',
                edge: JSON.parse(JSON.stringify(edges.get(data_element.id)))
            });
...
function addEntityParent(idTo, action, idSelected) {
    ...
    var data_element = {
        width: 3, from: parseInt(idSelected), to: parseInt(idTo), type: "parent", arrows:
            {
    ...
    if (idNodo != null) {
        ...
        dataFull.forEach(function (key) {
            if (edges.get(key).type == "child")
                data.push({
                    id: key,
                    labelChild: nodes.get(edges.get(key).to).label,
                    idChild: nodes.get(edges.get(key).to).id
                });
        });
    }
    return data;
...
function addEntityChild(idTo, action, idSelected) {
    ...
    var data_element = {
        from: parseInt(idSelected), type: "child", to: parseInt(idTo), arrows:
            {
...
function addSubAttribute(name, action, idSelected, idAttribute = idEntity, comp, notNll, uniq, multi, dom, sz) {
    ...
    var data_element = {
        ...
        labelBackend: name,
        type: 'subAttribute',
        borderWidth: word_multi,
        label: word_pk,
        dataAttribute: {composite: comp, notNull: notNll, unique: uniq, multivalued: multi, domain: dom, size: sz},
        shape: 'ellipse',
        IsSuperEntity: false,
        superEntity: -1,
        color: '#4de4fc',
    ...
    if (nodes.get(nodo_select).shape !== 'ellipse') {
        ...
        nodos.forEach(function (edg) {
            ...
            if (nodes.get(idNodo).shape == "ellipse") {
                data.push({
                    id: idNodo,
                    label: nodes.get(idNodo).labelBackend,
                    type: nodes.get(idNodo).dataAttribute.domain,
                    size: nodes.get(idNodo).dataAttribute.size
                });
            }
        });
    }
...
function allAttributeOfEntitySuper(nodo_select) {
    ...
    nodos.forEach(function (edg) {
        ...
        if (nodes_super.get(idNodo).shape == "ellipse")
            data.push({
                id: idNodo,
                label: nodes_super.get(idNodo).labelBackend,
                type: nodes_super.get(idNodo).dataAttribute.domain,
                size: nodes_super.get(idNodo).dataAttribute.size
            });
    });
    return data;
}
