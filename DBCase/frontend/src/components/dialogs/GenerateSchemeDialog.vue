...
<template>
  ...
  >
    <div class="grid gap-2 grid-rows-2">
      <Button @click="panelStore.open(PanelId.ERScheme)">{{ t('schema.generateER') }}</Button>
      <Button @click="panelStore.open(PanelId.LogicalScheme)">{{
        t('schema.generateLogical')
      }}</Button>
      <Button @click="panelStore.open(PanelId.BDScheme)">{{ t('schema.generatePhysical') }}</Button>
    </div>
  </Dialog>
</template>
