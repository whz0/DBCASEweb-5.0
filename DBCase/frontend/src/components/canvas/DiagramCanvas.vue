  ...
  if (isEntity) {
    return [
      ...
      },
      {
        ...
        icon: 'bi bi-pencil',
        command: () => dialogStore.open(DialogId.Rename),
      },
      { separator: true },
      {
...
const onContextMenu = (event: MouseEvent) => {
  ...
  const stage = stageRef.value.getStage() as unknown as Stage
...
const getLineRectangleIntersection = (
  p1: Position,
  p2: Position,
  rect: RectShape,
): Position | null => {
...
const getLineEllipseIntersection = (
  p1: Position,
  p2: Position,
...
const getLineDiamondIntersection = (
  p1: Position,
  p2: Position,
...
const getLineSegmentIntersection = (
  p1: Position,
  p2: Position,
  p3: Position,
  p4: Position,
...
const getLineTriangleIntersection = (
  p1: Position,
  p2: Position,
    ...
    if (parentEntity || parentAttr) {
      ...
      if (startPoint && endPoint) {
        ...
        connections.push({
          ...
          endX: parentEntity?.position.x || parentAttr?.position.x || 0,
          endY: parentEntity?.position.y || parentAttr?.position.y || 0,
        })
      }
    }
  })
